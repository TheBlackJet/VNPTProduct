...
function add_pb_to_content($content)
{
    ...
    if (!post_password_required()) {
        $custom_content = get_pb_parser((isset($pagebuilder['modules']) ? $pagebuilder['modules'] : array()));
    }
...
function get_selected_pf_images_for_admin($pagebuilder)
{
    ...
    if (isset($pagebuilder['post-formats']['images']) && is_array($pagebuilder['post-formats']['images'])) {
        foreach ($pagebuilder['post-formats']['images'] as $imgid => $img) {
            $compile .= "<div class='img-item style_small'><div class='img-preview'><img src='" . aq_resize($img['src'], "62", "62", true) . "' data-full-url='{$img['src']}' data-timthumb-url='" . aq_resize($img['src'], "156", "106", true, true, true) . "' alt='' class='previmg'><div class='hover-container'></div><div class='deldel-container'></div></div><input type='hidden' name='pagebuilder[post-formats][images][" . $imgid . "][src]' value='{$img['src']}'></div>";
        }
    }
    return $compile;
}
            ...
            if ($style == "small") {
                ...
                <div class='img-item style_small available_media_item'>
                    <div class='img-preview'>
                        <img class='previmg' alt='' data-timthumb-url='" . aq_resize($media_url, "156", "106", true, true, true) . "' data-full-url='" . $media_url . "' src='" . aq_resize($media_url, "62", "62", true, true, true) . "'>
...
{
    array_walk_recursive($pagebuilderArray, 'before_save_pagebuilder_array');
...
function get_plugin_pagination($range = 10, $type = "")
{
    ...
    if ($max_page > 1) {
        $compile .= '<ul class="gt3_pagerblock">';
    }
...
function gt3pb_check_http($string)
{
